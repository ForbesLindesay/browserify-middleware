...
function bundleModule(modules, options) {
  ...
  for (var i = 0; i < modules.length; i++) {
    if (modules[i].constructor === Object) { // obj spec support; i.e., {jquery:"jquery-browserify"} or {module:"jquery", options:{...}}
        var spec = modules[i],
            keys = Object.keys(spec);
        if (keys.length === 1) { //short-form options
            b.require(spec[keys[0]], {expose: keys[0]});
        } else { // long-form options
            b.require(spec.module, spec.options);
        }
    } else {
        b.require(modules[i]);
    }
