...
function bundleModule(modules, options) {
  ...
  function baseDirFn(moduleName) {
    var pkg = resolve.sync(moduleName, {basedir : options.basedir});
    b.require(pkg, {expose : moduleName});
  }
  function callerDirFn(moduleName) {
    b.require(modules[i]);
  }
  var fn = options.basedir ? baseDirFn : callerDirFn;
  var i;
